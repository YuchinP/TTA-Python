...
#remember to import other modules
import phonebook_main
import phonebook_gui


def center_window(self, w, h):#pass in the tkinter frame (master) reference and the
    #w and h get the user's width and height screen
...
def addToList(self):
    ...
    var_fullname = ("{} {}".format(var_fname, var_lname)) #combine into full name
...
def onDelete(self):
    ...
    with conn:
        ...
        #in the database... cannot delete the last record or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
...
def onRefresh(self):
    #populate the listboxm coinciding with the database
...
def onUpdate(self):
    try:
        ...
        var_value = self.lstList1.get(var_select) #lst selections text value
    except:
        messagebox.showinfo("missing selection", "no name was selected from the listbox. \n cancelling the update reques.")
        return
    #user will only be allowed to update changes for phone and emails
    #for name changesm the user will need to delete the entire record and start over
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count2 == 0: #if the proposed changes aren't already in the database, then proceed
