...
def is_it_closed(x):
    ...
    branch_close = time(hour=21,minute=0,second=0)
...
P = utc.localize(utc_time).astimezone(tz)
NY = utc.localize(utc_time).astimezone(etz)
LON = utc.localize(utc_time).astimezone(ltz)
